...
export default function UsuariosPage() {
  ...
  async function handleAsignar(usuarioId: string) {
    ...
    if (editando) {
      // Edición: una sola empresa, la que ya se estaba editando.
      const res = await fetch(`/api/usuarios/${usuarioId}/asignaciones`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...formAsignar, empresaId: empresaIdEditando }),
      });
      if (!res.ok) {
        const data = await res.json();
        setError(data.error?.toString() ?? "No se pudo guardar el cambio.");
        return;
      }
    } else {
      // Nueva asignación: puede ser a varias empresas a la vez, con el
      // mismo rol y los mismos permisos para todas.
      if (formAsignar.empresaIds.length === 0) {
        setError("Marca al menos una empresa.");
        return;
      }
      const resultados = await Promise.all(
        formAsignar.empresaIds.map((empresaId) =>
          fetch(`/api/usuarios/${usuarioId}/asignaciones`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ ...formAsignar, empresaId }),
          })
        )
      );
      const fallidas = resultados.filter((r) => !r.ok);
      if (fallidas.length > 0) {
        setError(`No se pudo asignar a ${fallidas.length} de ${resultados.length} empresa(s) seleccionadas.`);
        return;
      }
    }
    ...
    setAsignandoEn(null);
    setEditando(null);
    setEmpresaIdEditando(null);
    setFormAsignar({ empresaIds: [], tipoActor: "cliente", rolOperativoNombre: "Admin Local", accesoTotal: true, permisos: [] });
    cargar();
  }
  ...
  function abrirEdicion(usuarioId: string, a: Asignacion) {
    ...
    setEmpresaIdEditando(a.empresaId);
            ...
            <div key={u.id} className="card">
              ...
              {asignandoEn === u.id && (
                <div style={{ marginTop: 12, borderTop: "1px solid var(--line)", paddingTop: 12 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: editando ? "1fr 1fr" : "1fr 1fr", gap: 10 }}>
                  ...
                  {!editando && (
                    <div className="field" style={{ marginTop: 10 }}>
                      <label>Empresas ({formAsignar.empresaIds.length} seleccionada{formAsignar.empresaIds.length !== 1 ? "s" : ""})</label>
                      <div style={{ border: "1px solid var(--line)", borderRadius: "var(--radius)", maxHeight: 180, overflowY: "auto" }}>
                        {empresas.map((e, i) => (
                          <label
                            key={e.id}
                            className="checkbox-row"
                            style={{ padding: "6px 10px", borderBottom: i < empresas.length - 1 ? "1px solid var(--line)" : "none", fontSize: 13 }}
                          >
                            <input
                              type="checkbox"
                              checked={formAsignar.empresaIds.includes(e.id)}
                        ...
                        {empresas.length === 0 && (
                          <p style={{ padding: 10, fontSize: 12, color: "var(--ink-soft)" }}>No hay empresas creadas todavía.</p>
                        )}
                      </div>
                      <p className="mono" style={{ fontSize: 10.5, color: "var(--ink-soft)", marginTop: 4 }}>
                        El rol y los permisos que elijas abajo se aplican igual a todas las empresas marcadas.
                      </p>
                    </div>
                  )}
                  ...
                  <div style={{ display: "flex", gap: 10, marginTop: 10 }}>
                    ...
                    <button className="btn-ghost" onClick={() => { setAsignandoEn(null); setEditando(null); setEmpresaIdEditando(null); }}>
                      Cancelar
                    </button>
                  </div>
                </div>
