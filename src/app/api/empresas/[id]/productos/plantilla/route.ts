import { NextResponse } from "next/server";
import * as XLSX from "xlsx";
import { prisma } from "@/lib/prisma";
import { getUsuarioActual, verificarAccesoEmpresa } from "@/lib/auth";

export const dynamic = "force-dynamic";

// GET /api/empresas/[id]/productos/plantilla — genera un Excel para
// cargar Productos en bloque, separado de la plantilla de Insumos.
//
// Producto = mercadería de venta al público comprada ya terminada para
// revender (gaseosa, agua, etc.), con su propio stock y costo — ya no se
// construye con una receta/ficha técnica, así que la plantilla trae una
// sola hoja de datos ("Productos") con las mismas columnas de
// stock/costo/unidad que ya tiene la de Insumos, más una hoja "Ayuda" con
// las categorías y unidades que YA EXISTEN en esta empresa.
export async function GET(request: Request, { params }: { params: { id: string } }) {
  const usuarioActual = await getUsuarioActual();
  if (!usuarioActual) return NextResponse.json({ error: "No autenticado" }, { status: 401 });

  const empresaId = BigInt(params.id);
  try {
    await verificarAccesoEmpresa(usuarioActual.id, empresaId, "productos");
  } catch (error) {
    return NextResponse.json({ error: (error as Error).message }, { status: 403 });
  }

  const [categorias, unidades] = await Promise.all([
    prisma.categoriaProducto.findMany({ where: { empresaId }, orderBy: { nombre: "asc" } }),
    prisma.unidadMedida.findMany({ where: { empresaId }, orderBy: { nombre: "asc" } }),
  ]);

  const encabezados = [
    "nombre",
    "codigo",
    "categoria",
    "tipo",
    "precio_venta",
    "unidad_medida",
    "stock_minimo",
    "stock_inicial",
    "costo_unitario_inicial",
  ];
  const filaEjemplo = ["Gaseosa 500ml", "PROD-001", "Bebidas", "producto", 3.5, "unidad", 6, 24, 2.2];

  const hojaProductos = XLSX.utils.aoa_to_sheet([encabezados, filaEjemplo]);
  hojaProductos["!cols"] = encabezados.map(() => ({ wch: 20 }));

  const hojaAyuda = XLSX.utils.aoa_to_sheet([
    ["Cómo llenar esta plantilla"],
    [""],
    ["nombre", "Obligatorio."],
    ["codigo", "Opcional, tu código interno."],
    ["categoria", "Opcional. Si escribes un nombre que no existe, se crea automáticamente."],
    ["tipo", "Obligatorio: escribe exactamente \"producto\" o \"servicio\"."],
    ["precio_venta", "Obligatorio, número (el precio al que se vende)."],
    ["unidad_medida", "Opcional. Escribe el NOMBRE completo (ej. 'unidad', 'caja'), no la abreviatura. Si no existe, se crea con la misma palabra como abreviatura."],
    ["stock_minimo", "Opcional, número. Para la alerta de stock bajo."],
    ["stock_inicial", "Opcional, número. Si pones más de 0, se crea un lote de apertura con ese costo."],
    ["costo_unitario_inicial", "Obligatorio SI pones stock_inicial > 0. Costo por unidad (lo que te costó comprarlo)."],
    [""],
    ["Categorías que ya existen en esta empresa:"],
    ...categorias.map((c) => [c.nombre]),
    [""],
    ["Unidades de medida que ya existen:"],
    ...unidades.map((u) => [`${u.nombre} (${u.abreviatura})`]),
  ]);
  hojaAyuda["!cols"] = [{ wch: 30 }, { wch: 70 }];

  const libro = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(libro, hojaProductos, "Productos");
  XLSX.utils.book_append_sheet(libro, hojaAyuda, "Ayuda");

  const buffer = XLSX.write(libro, { type: "buffer", bookType: "xlsx" });

  return new NextResponse(new Uint8Array(buffer), {
    headers: {
      "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      "Content-Disposition": 'attachment; filename="plantilla-productos.xlsx"',
    },
  });
}
