...
/**
 * Verifica que el usuario tenga acceso a una empresa puntual y devuelve
 * su rol operativo ahí (para aplicar la matriz de permisos, sección 2 de
 * la arquitectura). Lanza error si no tiene acceso — usar en cada API route
 * que reciba un empresaId.
 */
export async function verificarAccesoEmpresa(usuarioId: string, empresaId: bigint) {
  const usuario = await prisma.usuario.findUnique({ where: { id: usuarioId } });
  if (!usuario) throw new Error("Usuario no encontrado");

  if (usuario.esSuperadminPlataforma) {
    return { rolOperativo: "Gerencial", tipoActor: "superadmin" as const };
  }
  ...
  return { rolOperativo: asignacion.rolOperativo.nombre, tipoActor: asignacion.tipoActor };
}
